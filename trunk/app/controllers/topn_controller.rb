...
#    @dayusages = Dayusagesummary.find_by_sql("select 0 as id, slice_id as slice_id, max(nitems) as nitems, sum(total_activity_minutes) as total_activity_minutes, avg(avg_cpu) as avg_cpu, avg(avg_send_BW) as avg_send_BW, avg(avg_recv_BW) as avg_recv_BW,  sum(total_send_BW) as total_send_BW, sum(total_cpu) as total_cpu, sum(total_send_BW) as total_send_BW, sum(total_recv_BW) as total_recv_BW, max(max_cpu) as max_cpu, max(max_send_BW) as max_send_BW, max(max_recv_BW) as max_recv_BW, sum(number_of_samples) as number_of_samples from dayusagesummaries where day > date_sub(now(), interval " + @days +" day) and slice_id != -1 group by slice_id order by sum(total_cpu) desc limit " + @count)
  ...
  def slices_by_send_bw
    ...
    vals = []
    @dayusages.each do |item|
      vals << PieValue.new((item.total_send_BW.to_f/1024),item.slice.name)
    end
    pie.set_values(vals)
    @chart = OpenFlashChart.new
    @chart.set_title(title)
  ...
  def slices_by_recv_bw
    ...
    @dayusages = Dayusage.find_by_sql("select 0 as id, slice_id, count(distinct node_id) as node_id, now() as day, sum(total_activity_minutes) as total_activity_minutes, avg(avg_cpu) as avg_cpu, avg(avg_send_BW) as avg_send_BW, avg(avg_recv_BW) as avg_recv_BW, sum(total_activity_minutes)*sum(total_cpu)/sum(number_of_samples)/100 as total_cpu,  60*sum(total_activity_minutes)*sum(total_send_BW)/sum(number_of_samples) as total_send_BW, 60*sum(total_activity_minutes)*sum(total_recv_BW)/sum(number_of_samples) as total_recv_BW, max(max_cpu) as max_cpu, max(max_send_BW) as max_send_BW, max(max_recv_BW) as max_recv_BW, sum(number_of_samples) as number_of_samples, max(last_update) as last_update from dayusages where day > date_sub(now(), interval " + @days +" day) group by slice_id order by 60*sum(total_activity_minutes)*sum(total_recv_BW)/sum(number_of_samples) desc limit " + @count)
    ...
    vals = []
    @dayusages.each do |item|
      vals << PieValue.new((item.total_recv_BW.to_f/1024),item.slice.name)
    end
    pie.set_values(vals)
    @chart = OpenFlashChart.new
    @chart.set_title(title)
  ...
  def nodes_by_send_bw
    ...
    vals = []
    @dayusages.each do |item|
      vals << PieValue.new((item.total_send_BW.to_f/1024),item.node.hostname)
    end
    pie.set_values(vals)
    @chart = OpenFlashChart.new
    @chart.set_title(title)
  ...
  def nodes_by_recv_bw
    ...
    vals = []
    @dayusages.each do |item|
      vals << PieValue.new((item.total_recv_BW.to_f/1024),item.node.hostname)
    end
    pie.set_values(vals)
    @chart = OpenFlashChart.new
    @chart.set_title(title)
